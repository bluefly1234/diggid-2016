...
function newCOL() {
    ...
    COL.transform = function(ct, obj) {
        if (COL.MatrixTransformMode && obj.matrix && obj.usematrix) {
            ...
            ct.translate(
                ((obj.relativeposition.x && pn) ? obj.relativeposition.x * obj.parentNode.width : obj.x) + rotatex - (obj.relativepositionpoint.x ? obj.relativepositionpoint.x * obj.width : obj.positionpoint.x), ((obj.relativeposition.y && pn) ? obj.relativeposition.y * obj.parentNode.height : obj.y) + rotatey - (obj.relativepositionpoint.y ? obj.relativepositionpoint.y * obj.height : obj.positionpoint.y));
        ...
        commonFunction: {
            ...
            New: function(inherit, newPositionObj, newZoom) { //获得一个可选继承和使用新位置对象的元素
                ...
                if (newPositionObj) {
                    ...
                    newobj.relativepositionpoint = {
                        x: 0,
                        y: 0
                    };
                    newobj.relativerotatecenter = {
                        x: 0,
                        y: 0
                    };
                }
                (newZoom !== false) && (newobj.zoom = {
                    x: 1,
                    y: 1
                });
                if (inherit === true) {
            ...
            setMatrix: function(floatarrayMatrix) {
                ...
                if (!floatarrayMatrix) {
                    ...
                    COL.tools.multiplyMatrix(
                        [1, 0, -rotatex, 0, 1, -rotatey, 0, 0, 0], [this.zoom.x, 0, 0, 0, this.zoom.y, 0, 0, 0, 0], [cos, -sin, 0, sin, cos, 0, 0, 0, 0], [1, 0, ((this.relativeposition.x && pn) ? this.relativeposition.x * this.parentNode.width : this.x) + rotatex - (this.relativepositionpoint.x ? this.relativepositionpoint.x * this.width : this.positionpoint.x), 0, 1, ((this.relativeposition.y && pn) ? this.relativeposition.y * this.parentNode.height : this.y) + rotatey - (this.relativepositionpoint.y ? this.relativepositionpoint.y * this.height : this.positionpoint.y), 0, 0, 0],
                        this.matrix);
                } else {
